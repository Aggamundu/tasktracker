-- Move a post-it to another cell index and/or row/column, preserving unique (sprint_id, row_index, column_key, position).
...
begin
  ...
  -- Cross cell: move row out of the way first (avoids duplicate (cell, position) when v_old_pos = 0),
  -- then close gap in source, make room in target, then assign.
  select coalesce(max(n."position"), 0) + 1000
  into v_temp
  from public.post_it_notes n
  where
